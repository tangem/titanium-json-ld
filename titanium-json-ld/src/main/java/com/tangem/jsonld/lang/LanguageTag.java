...
public final class LanguageTag {
    ...
    public static boolean isWellFormed(final String languageTag) {
        ...
        if (JavaOver8Utils.isBlank(languageTag)) {
            return false;
        }

        int[] chars = languageTag.trim().codePoints().toArray();

        if (com.tangem.rdf.lang.RdfAlphabet.ASCII_ALPHA.negate().test(chars[0])) {
            return false;
        }
        ...
        if (com.tangem.rdf.lang.RdfAlphabet.ASCII_ALPHA_NUM.negate().test(chars[chars.length - 1])) {
            return false;
        }
