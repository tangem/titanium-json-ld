...
public final class FrameMatcher {
    ...
    // required
    private com.tangem.jsonld.framing.FramingState state;
    private com.tangem.jsonld.framing.Frame frame;
    private boolean requireAll;
    
    private FrameMatcher(com.tangem.jsonld.framing.FramingState state, com.tangem.jsonld.framing.Frame frame, boolean requireAll) {
        this.state = state;
        this.frame = frame;
        this.requireAll = requireAll; 
    }
    
    public static final FrameMatcher with(FramingState state, com.tangem.jsonld.framing.Frame frame, boolean requireAll) {
        return new FrameMatcher(state, frame, requireAll);
    }
    
    public List<String> match(final Collection<String> subjects) throws com.tangem.jsonld.api.JsonLdError {
    ...
    public boolean match(final Map<String, JsonValue> node) throws JsonLdError {
        ...
        for (final String property : frame.keys()) {
            ...
            final com.tangem.jsonld.framing.Frame propertyFrame;

            if (JsonUtils.isNotNull(propertyValue) 
                    && JsonUtils.isArray(propertyValue) && JsonUtils.isNotEmptyArray(propertyValue)) {
                
                propertyFrame = com.tangem.jsonld.framing.Frame.of((JsonStructure)propertyValue);
                    ...
                    if (!nodeValues.isEmpty() && ListObject.isListObject(nodeValues.get(0))) {
                        ...
                        if (ValueObject.isValueObject(listValue.asJsonArray().get(0))) {
                            ...
                            for (JsonValue value : JsonUtils.toJsonArray(nodeListValue)) {
                                ...
                                match = com.tangem.jsonld.framing.Frame.of((JsonStructure)listValue).matchValue(value);
                                if (match) {
                                    break;
                                }
                            }
